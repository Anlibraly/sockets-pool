...
function Pool(servers, opts) {
  ...
  this.keepAliveTime = (opts && opts.keepAliveTime) || 5000;
...
Pool.prototype._remove = function(socket) {
  var newAvailable = [];
  
  for(var soc of this.available) {
    if(soc == socket) {
      socket.end();
    }else {
      newAvailable.push(soc);
    }
  }
...
Pool.prototype._ensure = function() {
  ...
  if (this.min > available_len && socket_len < this.max) {
    ...
    if(this.keepAlive){
        sock.setKeepAlive(true, this.keepAliveTime);
    }
